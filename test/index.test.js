...
const normalize = (str) => minify(str).css;

async function run(input, output, opts, from) {
  let result = await postcss([nestedImport(opts)]).process(input, { from });
...
test("05 - empty import", async () => {
  ...
  match(error.message, /invalid usage - no param/i);
});
...
test("06 - throws with a meaningful message when fs error happens", async () => {
  ...
  match(error.message, /error reading file/i);
